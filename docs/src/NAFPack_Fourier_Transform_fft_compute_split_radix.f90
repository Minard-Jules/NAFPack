...
contains
    ...
    pure subroutine process_L_butterflies(result, plan, stage_params, &
        ...
        complex(sp) :: temp_diff_even, temp_diff_odd
        ...
        do i0 = start_index, plan%N - 1, stride
            ...
            case (DIT%id)
                ! radix-2
                temp_radix2 = even2 * twiddles_wk + odd2 * twiddles_w3k
                result(i0) = even1 + temp_radix2
                result(i2) = even1 - temp_radix2

                ! radix-4
                temp_radix4 = even2 * twiddles_wk - odd2 * twiddles_w3k
                result(i1) = odd1 - im_sp * temp_radix4
                result(i3) = odd1 + im_sp * temp_radix4
            case (DIF%id)
                ! radix-2
                result(i0) = even1 + even2
                result(i1) = odd1 + odd2

                ! radix-4
                temp_diff_even = even1 - even2
                temp_diff_odd = odd1 - odd2
                result(i2) = temp_diff_even - im_sp * temp_diff_odd * twiddles_wk
                result(i3) = temp_diff_even + im_sp * temp_diff_odd * twiddles_w3k
            end select
