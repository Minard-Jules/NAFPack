module test_Fourier_Transform_dft2_complex
...
contains
    ...
    subroutine collect_Fourier_Transform_dft2(testsuite)
        ...
        testsuite = [ &
                    new_unittest("dft complex(sp)  2D", test_cmplx_dft2_sp), &
                    new_unittest("idft complex(sp) 2D", test_cmplx_idft2_sp), &
                    new_unittest("dft complex(dp)  2D", test_cmplx_dft2_dp), &
                    new_unittest("idft complex(dp) 2D", test_cmplx_idft2_dp), &
                    new_unittest("dft complex(qp)  2D", test_cmplx_dft2_qp), &
                    new_unittest("idft complex(qp) 2D", test_cmplx_idft2_qp) &
                    ]

    end subroutine collect_Fourier_Transform_dft2

    subroutine test_cmplx_dft2_sp(error)
        ...
        complex(sp), dimension(number_tests, Nx, Ny) :: signal
        complex(sp), dimension(Nx, Ny) :: signal_used
        ...
        do i = 1, Nx
            do j = 1, Ny
                signal(2, i, j) = exp(im_sp * freqx * xlist(i) + im_sp * freqy * ylist(j))
            end do
        end do

        fs_exact(2, :, :) = (0._sp, 0._sp)
        fs_exact(2, freqx + 1, freqy + 1) = cmplx(real(Nx * Ny, kind=sp), 0._sp, kind=sp)
    ...
    end subroutine test_cmplx_dft2_sp

    subroutine test_cmplx_idft2_sp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        real(sp), dimension(Nx) :: xlist
        real(sp), dimension(Ny) :: ylist
        complex(sp), dimension(number_tests, Nx, Ny) :: fs
        complex(sp), dimension(Nx, Ny) :: fs_used
        ...
        freqy = 1
        fs(2, :, :) = (0._sp, 0._sp)
        fs(2, freqx + 1, freqy + 1) = cmplx(real(Nx * Ny, kind=sp), 0._sp, kind=sp)

        freqx = 2
        freqy = 1
        do i = 1, Nx
            do j = 1, Ny
                signal_exact(2, i, j) = exp(im_sp * freqx * xlist(i) + im_sp * freqy * ylist(j))
            end do
        end do
    ...
    end subroutine test_cmplx_idft2_sp

    subroutine test_cmplx_dft2_dp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        complex(dp), dimension(number_tests, Nx, Ny) :: signal
        complex(dp), dimension(Nx, Ny) :: signal_used
        ...
        do i = 1, Nx
            do j = 1, Ny
                signal(2, i, j) = exp(im_dp * freqx * xlist(i) + im_dp * freqy * ylist(j))
            end do
        end do
        fs_exact(2, :, :) = (0._dp, 0._dp)
        fs_exact(2, freqx + 1, freqy + 1) = cmplx(real(Nx * Ny, kind=dp), 0._dp, kind=dp)
    ...
    end subroutine test_cmplx_dft2_dp

    subroutine test_cmplx_idft2_dp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        real(dp), dimension(Nx) :: xlist
        real(dp), dimension(Ny) :: ylist
        complex(dp), dimension(number_tests, Nx, Ny) :: fs
        complex(dp), dimension(Nx, Ny) :: fs_used
        ...
        freqy = 1
        fs(2, :, :) = (0._dp, 0._dp)
        fs(2, freqx + 1, freqy + 1) = cmplx(real(Nx * Ny, kind=dp), 0._dp, kind=dp)
        do i = 1, Nx
            do j = 1, Ny
                signal_exact(2, i, j) = exp(im_dp * freqx * xlist(i) + im_dp * freqy * ylist(j))
            end do
        end do
    ...
    end subroutine test_cmplx_idft2_dp

    subroutine test_cmplx_dft2_qp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        complex(qp), dimension(number_tests, Nx, Ny) :: signal
        complex(qp), dimension(Nx, Ny) :: signal_used
        ...
        do i = 1, Nx
            do j = 1, Ny
                signal(2, i, j) = exp(im_qp * freqx * xlist(i) + im_qp * freqy * ylist(j))
            end do
        end do
        fs_exact(2, :, :) = (0._qp, 0._qp)
        fs_exact(2, freqx + 1, freqy + 1) = cmplx(real(Nx * Ny, kind=qp), 0._qp, kind=qp)
    ...
    end subroutine test_cmplx_dft2_qp

    subroutine test_cmplx_idft2_qp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        real(qp), dimension(Nx) :: xlist
        real(qp), dimension(Ny) :: ylist
        complex(qp), dimension(number_tests, Nx, Ny) :: fs
        complex(qp), dimension(Nx, Ny) :: fs_used
        ...
        freqy = 1
        fs(2, :, :) = (0._qp, 0._qp)
        fs(2, freqx + 1, freqy + 1) = cmplx(real(Nx * Ny, kind=qp), 0._qp, kind=qp)
        do i = 1, Nx
            do j = 1, Ny
                signal_exact(2, i, j) = exp(im_qp * freqx * xlist(i) + im_qp * freqy * ylist(j))
            end do
        end do
    ...
    end subroutine test_cmplx_idft2_qp

end module test_Fourier_Transform_dft2_complex

program test
    use, intrinsic :: iso_fortran_env, only: error_unit, output_unit
    use testdrive, only: run_testsuite, new_testsuite, testsuite_type, init_color_output
    use test_Fourier_Transform_dft2_complex
    ...
    testsuites = [ &
                 new_testsuite("Fourier Transform DFT 2D complex", collect_Fourier_Transform_dft2) &
                 ]
