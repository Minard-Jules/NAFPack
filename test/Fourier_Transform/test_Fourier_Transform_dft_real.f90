module test_Fourier_Transform_dft_real
...
contains
    ...
    subroutine collect_Fourier_Transform_dft(testsuite)
        ...
        testsuite = [ &
                    new_unittest("dft real sp 1D", test_real_dft_sp), &
                    new_unittest("idft real sp 1D", test_real_idft_sp), &
                    new_unittest("dft real dp 1D", test_real_dft_dp), &
                    new_unittest("idft real dp 1D", test_real_idft_dp), &
                    new_unittest("dft real qp 1D", test_real_dft_qp), &
                    new_unittest("idft real qp 1D", test_real_idft_qp) &
                    ]

    end subroutine collect_Fourier_Transform_dft

    subroutine test_real_dft_sp(error)
        ...
        real(sp), dimension(number_tests, Nx) :: signal
        real(sp), dimension(Nx) :: signal_used
        ...
        freq = 2
        signal(2, :) = cos(freq * xlist)

        fs_exact(2, :) = (0._sp, 0._sp)
        fs_exact(2, freq + 1) = cmplx(real(Nx, kind=sp) / 2, 0._sp, kind=sp)
        fs_exact(2, Nx - freq + 1) = cmplx(real(Nx, kind=sp) / 2, 0._sp, kind=sp)
    ...
    end subroutine test_real_dft_sp

    subroutine test_real_idft_sp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        type(LoopMethod) :: loop_method
        real(sp), dimension(Nx) :: xlist
        real(sp), dimension(number_tests, Nx) :: fs
        real(sp), dimension(Nx) :: fs_used
        ...
        freq = 2
        fs(2, :) = 0._sp
        fs(2, freq + 1) = real(Nx, kind=sp) / 2
        fs(2, Nx - freq + 1) = real(Nx, kind=sp) / 2

        signal_exact(2, :) = cmplx(cos(freq * xlist), 0.0_sp, kind=sp)

        ! test 3: constant signal (DC component)
        fs(3, :) = 0._sp
        fs(3, 1) = real(Nx, kind=sp)
    ...
    end subroutine test_real_idft_sp

    subroutine test_real_dft_dp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        real(dp), dimension(number_tests, Nx) :: signal
        real(dp), dimension(Nx) :: signal_used
        ...
        fs_exact(2, freq + 1) = cmplx(real(Nx, kind=dp) / 2, 0._dp, kind=dp)
        fs_exact(2, Nx - freq + 1) = cmplx(real(Nx, kind=dp) / 2, 0._dp, kind=dp)
    ...
    end subroutine test_real_dft_dp

    subroutine test_real_idft_dp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        type(LoopMethod) :: loop_method
        real(dp), dimension(Nx) :: xlist
        real(dp), dimension(number_tests, Nx) :: fs
        real(dp), dimension(Nx) :: fs_used
        ...
        ! test 1: Dirac delta function
        fs(1, :) = 1._dp
        signal_exact(1, :) = cmplx(0._dp, 0._dp, kind=dp)
        signal_exact(1, 1) = cmplx(1._dp, 0._dp, kind=dp)

        ! test 2: sinusoidal signal of frequency 2
        freq = 2
        fs(2, :) = 0._dp
        fs(2, freq + 1) = real(Nx, kind=dp) / 2
        fs(2, Nx - freq + 1) = real(Nx, kind=dp) / 2
        signal_exact(2, :) = cmplx(cos(freq * xlist), 0._dp, kind=dp)

        ! test 3: constant signal (DC component)
        fs(3, :) = 0._dp
        fs(3, 1) = real(Nx, kind=dp)
        signal_exact(3, :) = cmplx(1._dp, 0._dp, kind=dp)
    ...
    end subroutine test_real_idft_dp

    subroutine test_real_dft_qp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        real(qp), dimension(number_tests, Nx) :: signal
        real(qp), dimension(Nx) :: signal_used
        ...
        fs_exact(2, freq + 1) = cmplx(real(Nx, kind=qp) / 2, 0._qp, kind=qp)
        fs_exact(2, Nx - freq + 1) = cmplx(real(Nx, kind=qp) / 2, 0._qp, kind=qp)
    ...
    end subroutine test_real_dft_qp

    subroutine test_real_idft_qp(error)
        type(error_type), allocatable, intent(out) :: error
        ...
        type(LoopMethod) :: loop_method
        real(qp), dimension(Nx) :: xlist
        real(qp), dimension(number_tests, Nx) :: fs
        real(qp), dimension(Nx) :: fs_used
        ...
        ! test 1: Dirac delta function
        fs(1, :) = 1._qp
        signal_exact(1, :) = cmplx(0._qp, 0._qp, kind=qp)
        signal_exact(1, 1) = cmplx(1._qp, 0._qp, kind=qp)

        ! test 2: sinusoidal signal of frequency 2
        freq = 2
        fs(2, :) = 0._qp
        fs(2, freq + 1) = real(Nx, kind=qp) / 2
        fs(2, Nx - freq + 1) = real(Nx, kind=qp) / 2
        signal_exact(2, :) = cmplx(cos(freq * xlist), 0._qp, kind=qp)

        ! test 3: constant signal (DC component)
        fs(3, :) = 0._qp
        fs(3, 1) = real(Nx, kind=qp)
        signal_exact(3, :) = cmplx(1._qp, 0._qp, kind=qp)
    ...
    end subroutine test_real_idft_qp

end module test_Fourier_Transform_dft_real

program test
    use, intrinsic :: iso_fortran_env, only: error_unit, output_unit
    use testdrive, only: run_testsuite, new_testsuite, testsuite_type, init_color_output
    use test_Fourier_Transform_dft_real
    ...
    testsuites = [ &
                 new_testsuite("Fourier Transform DFT 1D real", collect_Fourier_Transform_dft) &
                 ]
