...
contains
    ...
    subroutine test_meshgrid_sp_3D(error)
        type(error_type), allocatable, intent(out) :: error

        integer, parameter :: nx = 3, ny = 2, nz = 4
        real(sp), dimension(:), allocatable :: x_vector, y_vector, z_vector
        ...
        call check_meshgrid_sp_3D_ij(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        deallocate(X, Y, Z)
        ...
        call check_meshgrid_sp_3D_xy(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        deallocate(X, Y, Z)
    ...
    end subroutine test_meshgrid_sp_3D

    subroutine check_meshgrid_sp_3D_ij(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        type(error_type), allocatable, intent(out) :: error
    ...
    end subroutine check_meshgrid_sp_3D_ij

    subroutine check_meshgrid_sp_3D_xy(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        type(error_type), allocatable, intent(out) :: error
        real(sp), dimension(:), intent(in) :: x_vector, y_vector, z_vector
        real(sp), dimension(:, :, :), intent(in) :: X, Y, Z
        integer, intent(in) :: nx, ny, nz
    ...
    end subroutine check_meshgrid_sp_3D_xy

    subroutine test_meshgrid_dp_3D(error)
        type(error_type), allocatable, intent(out) :: error

        integer, parameter :: nx = 3, ny = 2, nz = 4
        real(dp), dimension(:), allocatable :: x_vector, y_vector, z_vector
        ...
        z_vector = [(real(i, dp), i=1, nz)]

        loop_method = init_loop_method(use_do_classic=.true.)
        call meshgrid(x_vector, y_vector, z_vector, X, Y, Z, indexing=INDEXING_IJ, loop_method=loop_method)
        call check_meshgrid_dp_3D_ij(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        deallocate(X, Y, Z)
        ...
        loop_method = init_loop_method(use_do_classic=.true.)
        call meshgrid(x_vector, y_vector, z_vector, X, Y, Z, indexing=INDEXING_XY, loop_method=loop_method)
        call check_meshgrid_dp_3D_xy(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        deallocate(X, Y, Z)
    ...
    end subroutine test_meshgrid_dp_3D

    subroutine check_meshgrid_dp_3D_ij(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        type(error_type), allocatable, intent(out) :: error
    ...
    end subroutine check_meshgrid_dp_3D_ij

    subroutine check_meshgrid_dp_3D_xy(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        type(error_type), allocatable, intent(out) :: error
        real(dp), dimension(:), intent(in) :: x_vector, y_vector, z_vector
        real(dp), dimension(:, :, :), intent(in) :: X, Y, Z
        integer, intent(in) :: nx, ny, nz
    ...
    end subroutine check_meshgrid_dp_3D_xy

    subroutine test_meshgrid_qp_3D(error)
        type(error_type), allocatable, intent(out) :: error

        integer, parameter :: nx = 3, ny = 2, nz = 4
        real(qp), dimension(:), allocatable :: x_vector, y_vector, z_vector
        real(qp), dimension(:, :, :), allocatable :: X, Y, Z
        type(LoopMethod) :: loop_method
        integer :: i
        ...
        z_vector = [(real(i, qp), i=1, nz)]

        loop_method = init_loop_method(use_do_classic=.true.)
        call meshgrid(x_vector, y_vector, z_vector, X, Y, Z, indexing=INDEXING_IJ, loop_method=loop_method)
        call check_meshgrid_qp_3D_ij(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        deallocate(X, Y, Z)
        ...
        loop_method = init_loop_method(use_do_classic=.true.)
        call meshgrid(x_vector, y_vector, z_vector, X, Y, Z, indexing=INDEXING_XY, loop_method=loop_method)
        call check_meshgrid_qp_3D_xy(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        deallocate(X, Y, Z)
    ...
    end subroutine test_meshgrid_qp_3D

    subroutine check_meshgrid_qp_3D_ij(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        type(error_type), allocatable, intent(out) :: error
        real(qp), dimension(:), intent(in) :: x_vector, y_vector, z_vector
        real(qp), dimension(:, :, :), intent(in) :: X, Y, Z
        integer, intent(in) :: nx, ny, nz
        integer :: i, j, k
    ...
    end subroutine check_meshgrid_qp_3D_ij

    subroutine check_meshgrid_qp_3D_xy(error, x_vector, y_vector, z_vector, X, Y, Z, nx, ny, nz)
        type(error_type), allocatable, intent(out) :: error
        real(qp), dimension(:), intent(in) :: x_vector, y_vector, z_vector
        real(qp), dimension(:, :, :), intent(in) :: X, Y, Z
        integer, intent(in) :: nx, ny, nz
    ...
    end subroutine check_meshgrid_qp_3D_xy

end module test_meshgrid_real_3D

program test
    use, intrinsic :: iso_fortran_env, only: error_unit, output_unit
    use testdrive, only: run_testsuite, new_testsuite, testsuite_type, init_color_output
    use test_meshgrid_real_3D
    ...
    testsuites = [ &
                 new_testsuite("meshgrid real 3D", collect_meshgrid) &
                 ]
