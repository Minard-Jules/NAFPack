...
recursive module function compute_fft_mixed_radix_recursive_cmplx_sp( &
    ...
    complex(sp), dimension(radix, size(signal)/radix) :: result_temp
    complex(sp), dimension(size(signal)/radix) :: twiddle
    integer(isp) :: N, next_radix, r
    ...
    case (DIT%id)
        if (stage - 1 >= 1) then
            next_radix = plan%radix_plan(stage - 1)
        else
            next_radix = 1
        end if

        do r = 0, radix - 1
            result_temp(r + 1, :) = compute_fft_mixed_radix_recursive_cmplx_sp( &
                                    signal(r + 1:N:radix), plan, stage - 1, next_radix) * twiddle**r
        end do

        result_temp = small_dft_kernel_sp(result_temp, radix, size(signal) / radix)

        do r = 0, radix - 1
            result(r * (N / radix) + 1:(r + 1) * (N / radix)) = result_temp(r + 1, :)
        end do
    case (DIF%id)
        if (stage <= size(plan%radix_plan)) then
            next_radix = plan%radix_plan(stage)
        else
            next_radix = size(plan%radix_plan)
        end if

        do r = 0, radix - 1
            result_temp(r + 1, :) = signal(r * (N / radix) + 1:(r + 1) * (N / radix))
        end do
        ...
        do r = 1, radix - 1
            result_temp(r + 1, :) = result_temp(r + 1, :) * twiddle**r
        end do

        do r = 0, radix - 1
            result(r+1:N:radix) = compute_fft_mixed_radix_recursive_cmplx_sp( &
                                result_temp(r + 1, :), plan, stage + 1, next_radix)
        end do
    ...
    do block_index = 0, stage_params%nb_blocks - 1
        ...
        do j = 0, stage_params%block_size - 1
            ...
            case (DIT%id)
                ...
                result_temp = reshape(small_dft_kernel_sp(reshape(result_temp, [radix, 1]), radix, 1), [radix])
...
pure function compute_vectorized_cmplx_sp( &
    ...
    do block_index = 0, stage_params%nb_blocks - 1
        ...
        case (DIT%id)
            ...
            result_temp = small_dft_kernel_sp(result_temp, plan%twiddles(stage_params%stage)%radix, stage_params%block_size)
...
pure function small_dft_kernel_sp(x, radix, block_size) result(y)
    complex(sp), dimension(:, :), intent(in) :: x
    integer(isp), intent(in) :: radix, block_size
